...
namespace DatThree
{
    class Program
    {
        static void Main(string[] args)
        {
           ...
           // var wire2 = "U62,R66,U55,R34,D71,R55,D58,R83";
            ...
            foreach (var match in crossover)
            {
                ...
                var match2 = path2.Single(p => p.X == match.X && p.Y == match.Y);

                var steps = match2.Steps + match.Steps;
                ...
                var distance = Math.Abs(match.X) + Math.Abs(match.Y);
        ...
        public class GridPointEquality : IEqualityComparer<GridPoint>
        {
            public bool Equals([AllowNull] GridPoint x, [AllowNull] GridPoint y)
            {
        ...
        static List<GridPoint> ParseInstructions(string input)
        {
            ...
            foreach (var command in inputList)
            {
                ...
                {
                    D = command[0],
                    V = int.Parse(command.Substring(1))
                };
            ...
            foreach (var command in commands)
            {
                ...
                {
                    case 'R':
                        for (int i = 0; i < command.V; i++)
                        {
                            path.Add(new GridPoint
                            {
                                X = x++,
                                Y = y,
                                Steps = steps++
